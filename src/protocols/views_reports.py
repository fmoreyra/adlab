...
class ReportCreateView(StaffRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        """Set protocol and histopathologist before saving."""
        protocol = self.get_protocol()
        form.instance.protocol = protocol
        form.instance.veterinarian = protocol.veterinarian
        # Set histopathologist from current user
        form.instance.histopathologist = (
...
class ReportPDFView(View):
    """
    Generate PDF version of a report.
    """
...
class ReportSendView(StaffRequiredMixin, FormView):
    """
    Send a finalized report to the veterinarian.
    """
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            messages.error(
                self.request,
                _("Error al enviar informe: %(error)s") % {"error": str(e)},
            )
...
def report_create_view(request, protocol_id):
    ...
    if protocol.status not in [
        ...
        messages.error(
            request,
            _(
                "El protocolo debe estar en estado 'Listo' para generar el informe."
            ),
        )
    ...
    if protocol.reports.exists():
        ...
        messages.warning(
            request, _("Ya existe un informe para este protocolo.")
        )
    ...
    if request.method == "POST" and form.is_valid():
        with transaction.atomic():
            ...
            messages.success(
                request,
                _(
                    "Informe creado exitosamente. Ahora puede agregar observaciones por cassette."
                ),
            )
    ...
    context = {
        "form": form,
        "protocol": protocol,
        "title": _("Crear Informe - %(protocol)s")
...
def report_edit_view(request, pk):
    ...
    if not report.can_edit():
        messages.error(
            request, _("Solo se pueden editar informes en estado borrador.")
        )
        return redirect("protocols:report_detail", pk=report.pk)
    ...
    context = {
        ...
        "report": report,
        "protocol": report.protocol,
        "title": _("Editar Informe - %(protocol)s")
        % {"protocol": report.protocol.protocol_number},
    }
...
def report_detail_view(request, pk):
    ...
    if not (request.user.is_staff or is_owner):
        messages.error(request, _("No tiene permisos para ver este informe."))
        return redirect("protocols:protocol_list")

    context = {
        "report": report,
        "protocol": report.protocol,
        "title": _("Informe - %(protocol)s")
        % {"protocol": report.protocol.protocol_number},
    }
...
def generate_report_pdf(report):
    ...
    if histopath.signature_image and os.path.exists(
        ...
        try:
            sig_img = Image(
                histopath.signature_image.path, width=2 * inch, height=1 * inch
            )
...
def report_finalize_view(request, pk):
    ...
    if report.status != Report.Status.DRAFT:
        messages.error(
            request, _("Solo se pueden finalizar informes en estado borrador.")
        )
        return redirect("protocols:report_detail", pk=report.pk)
    ...
    try:
        with transaction.atomic():
            ...
            messages.success(
                request,
                _(
                    "Informe finalizado exitosamente. El PDF ha sido generado y enviado por email."
                ),
            )
            return redirect("protocols:report_detail", pk=report.pk)
...
def report_pdf_view(request, pk):
    ...
    if not report.pdf_path or not os.path.exists(report.pdf_path):
        messages.error(
            request,
            _(
                "El PDF del informe no está disponible. Debe finalizar el informe primero."
            ),
        )
        return redirect("protocols:report_detail", pk=report.pk)
    ...
    try:
        ...
        messages.error(request, _("Error al cargar el PDF del informe."))
        return redirect("protocols:report_detail", pk=report.pk)
...
def report_send_view(request, pk):
    ...
    if not report.pdf_path or not os.path.exists(report.pdf_path):
        messages.error(request, _("El PDF del informe no está disponible."))
        return redirect("protocols:report_detail", pk=report.pk)
    ...
    if request.method == "POST" and form.is_valid():
        try:
            additional_email = form.cleaned_data.get(
                "additional_email", ""
            ).strip()
            custom_message = form.cleaned_data.get(
                "custom_message", ""
            ).strip()

            # Prepare email
            subject = _("Informe Histopatológico - Protocolo %(number)s") % {
            ...
            with open(report.pdf_path, "rb") as f:
                email.attach(
                    report.generate_pdf_filename(), f.read(), "application/pdf"
                )
            ...
            messages.success(
                request,
                _("Informe enviado exitosamente a %(email)s")
    ...
    context = {
        ...
        "report": report,
        "protocol": report.protocol,
        "title": _("Enviar Informe - %(protocol)s")
        % {"protocol": report.protocol.protocol_number},
    }

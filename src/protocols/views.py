...
def reception_confirm_view(request, pk):
    ...
    if discrepancies:
        _send_discrepancy_alert_email(protocol, discrepancies, sample_condition)
...
def reception_label_pdf_view(request, pk):
    ...
    protocol_url = request.build_absolute_uri(
        reverse('protocols:protocol_public_detail', kwargs={'external_id': protocol.external_id})
    )
    ...
    animal_text = protocol.animal_identification[:10] if len(protocol.animal_identification) > 10 else protocol.animal_identification
    p.drawString(text_x, qr_y + qr_size - 6 * mm, animal_text)
    
    # Species (third line)
    species_text = protocol.species[:8] if len(protocol.species) > 8 else protocol.species
    ...
    filter_fields = [
        {
            ...
            "type": "select",
            "placeholder": "Todos los tipos",
            "choices": [("all", "Todos"), ("cytology", "Citología"), ("histopathology", "Histopatología")],
            "value": analysis_type,
        },
    ]

...
class EmailNotificationTest(TestCase):
    ...
    def test_queue_email_with_protocol(self, mock_send_email):
        ...
        self.assertEqual(email_log.email_type, EmailLog.EmailType.SAMPLE_RECEPTION)
    ...
    def test_send_verification_email(self, mock_queue_email):
        ...
        self.assertEqual(call_args[1]['email_type'], EmailLog.EmailType.EMAIL_VERIFICATION)
        self.assertEqual(call_args[1]['recipient_email'], self.veterinarian_user.email)
        self.assertEqual(call_args[1]['subject'], "Verifique su correo electrónico - AdLab Veterinario")
        self.assertEqual(call_args[1]['context']['user'], self.veterinarian_user)
        self.assertEqual(call_args[1]['context']['verification_url'], verification_url)
    ...
    def test_send_password_reset_email(self, mock_queue_email):
        ...
        mock_queue_email.assert_called_once()
        call_args = mock_queue_email.call_args
        self.assertEqual(call_args[1]['email_type'], EmailLog.EmailType.PASSWORD_RESET)
        self.assertEqual(call_args[1]['recipient_email'], self.veterinarian_user.email)
        self.assertEqual(call_args[1]['subject'], "Restablezca su contraseña - AdLab Veterinario")
        self.assertEqual(call_args[1]['context']['user'], self.veterinarian_user)
    ...
    @patch('protocols.emails.queue_email')
    def test_send_sample_reception_notification_with_preferences(self, mock_queue_email):
        ...
        mock_queue_email.assert_called_once()
        call_args = mock_queue_email.call_args
        self.assertEqual(call_args[1]['email_type'], EmailLog.EmailType.SAMPLE_RECEPTION)
        self.assertEqual(call_args[1]['recipient_email'], "alt@example.com")  # Alternative email
        self.assertEqual(call_args[1]['subject'], f"Muestra recibida - Protocolo {self.protocol.protocol_number}")
        self.assertEqual(call_args[1]['protocol'], self.protocol)
        self.assertEqual(call_args[1]['veterinarian'], self.veterinarian)

    @patch('protocols.emails.queue_email')
    def test_send_sample_reception_notification_preferences_disabled(self, mock_queue_email):
    ...
    @patch('protocols.emails.queue_email')
    def test_send_sample_reception_notification_default_preferences(self, mock_queue_email):
        ...
        mock_queue_email.assert_called_once()
        call_args = mock_queue_email.call_args
        self.assertEqual(call_args[1]['recipient_email'], self.veterinarian.email)

    @patch('protocols.emails.queue_email')
    def test_send_report_ready_notification_with_attachment(self, mock_queue_email):
        ...
        mock_queue_email.assert_called_once()
        call_args = mock_queue_email.call_args
        self.assertEqual(call_args[1]['email_type'], EmailLog.EmailType.REPORT_READY)
        self.assertEqual(call_args[1]['attachment_path'], report_pdf_path)
        self.assertEqual(call_args[1]['context']['has_attachment'], True)

    @patch('protocols.emails.queue_email')
    def test_send_report_ready_notification_no_attachment(self, mock_queue_email):
    ...
    @patch('protocols.emails.queue_email')
    def test_send_work_order_notification_single_veterinarian(self, mock_queue_email):
        ...
        call_args = mock_queue_email.call_args
        self.assertEqual(call_args[1]['email_type'], EmailLog.EmailType.WORK_ORDER)
    ...
    @patch('protocols.emails.queue_email')
    def test_send_work_order_notification_multiple_veterinarians(self, mock_queue_email):

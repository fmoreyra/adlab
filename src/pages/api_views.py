...
class ManagementDashboardRequiredMixin(UserPassesTestMixin):
    ...
    def test_func(self):
        ...
        return user.is_authenticated and (
            user.is_lab_staff or 
            user.is_histopathologist or 
            user.is_admin_user
        )


class DashboardWIPView(LoginRequiredMixin, ManagementDashboardRequiredMixin, View):
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return render(request, 'pages/api/wip_widget.html', {
    ...
    def _calculate_wip_metrics(self) -> Dict:
        ...
        protocols_by_status = Protocol.objects.filter(
            status__in=[
        ...
        ).values('analysis_type', 'status').annotate(
            count=Count('id')
        )
            ...
            if status in status_mapping:
                ...
                if tipo == 'histopathology':
                    if stage == 'procesando':
                        ...
                        wip_data['histopatologia'][stage] = self._get_histopathology_processing_stages()
                    else:
        ...
        wip_data['histopatologia']['procesando'] = self._get_histopathology_processing_stages()
    ...
    def _get_histopathology_processing_stages(self) -> Dict:
        ...
        cassette_stages = Cassette.objects.filter(
            estado__in=[
        ...
        slide_stages = Slide.objects.filter(
            estado__in=[
...
class DashboardVolumeView(LoginRequiredMixin, ManagementDashboardRequiredMixin, View):
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return render(request, 'pages/api/volume_widget.html', {
    ...
    def _calculate_volume_metrics(self, periodo: str, tipo: str) -> Dict:
        ...
        report_filter = Q(updated_at__gte=date_from, status=Report.Status.FINALIZED)
        ...
        protocol_counts = Protocol.objects.filter(protocol_filter).values(
            'analysis_type'
        ).annotate(count=Count('id'))
        
        # Get report counts by type
        report_counts = Report.objects.filter(report_filter).values(
...
class DashboardTATView(LoginRequiredMixin, ManagementDashboardRequiredMixin, View):
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return render(request, 'pages/api/tat_widget.html', {
        ...
        for analysis_type in ['histopathology', 'cytology']:
            ...
            for report in reports:
                if report.protocol.reception_date:
                    tat = (report.updated_at.date() - report.protocol.reception_date).days
                    tat_days.append(tat)
            ...
            if tat_days:
                ...
                result_key = 'histopatologia' if analysis_type == 'histopathology' else 'citologia'
                
                # Calculate metrics
                result[result_key]['tat_promedio_dias'] = round(sum(tat_days) / n, 1)
                ...
                within_target = sum(1 for days in tat_days if days <= target_days)
...
class DashboardProductivityView(LoginRequiredMixin, ManagementDashboardRequiredMixin, View):
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return render(request, 'pages/api/productivity_widget.html', productivity_data)
        except Exception as e:
            return render(request, 'pages/api/productivity_widget.html', {
    ...
    def _calculate_productivity_metrics(self, periodo: str) -> Dict:
        ...
        for histo in histopathologists:
            ...
            avg_tat = round(sum(tat_days) / len(tat_days), 1) if tat_days else 0
        ...
        histopathologist_data.sort(key=lambda x: x['informes_enviados'], reverse=True)
...
class DashboardAgingView(LoginRequiredMixin, ManagementDashboardRequiredMixin, View):
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return render(request, 'pages/api/aging_widget.html', {
        ...
        for protocol in active_protocols:
            ...
            if days_since_reception > target_days:
                overdue_protocols.append({
                    ...
                    'veterinario': protocol.veterinarian.get_full_name() or protocol.veterinarian.user.email
                })
...
class DashboardAlertsView(LoginRequiredMixin, ManagementDashboardRequiredMixin, View):
    """
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return render(request, 'pages/api/alerts_widget.html', {

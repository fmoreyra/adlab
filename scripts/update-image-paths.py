...
def main():
    ...
    mode = (
        ...
        if dry_run
        else "UPDATE MODE (will modify files)"
    )
    ...
    if not dry_run:
        confirm = input(
            colorize(
                "⚠️  This will modify files. Continue? (yes/no): ", Colors.RED
            )
        )
    ...
    for file_path in markdown_files:
        ...
        if result["placeholders"] > 0:
            ...
            print(
                colorize(
                    f"   {result['placeholders']} placeholder(s) found",
                    Colors.YELLOW,
                )
            )
    ...
    print(
        colorize(f"📊 Files with placeholders: {updated_files}", Colors.BLUE)
    )
    print(
        colorize(f"📊 Total placeholders: {total_placeholders}", Colors.BLUE)
    )
    ...
    if dry_run:
        ...
        print(
            colorize(
                "   Run with option 2 to actually update files.", Colors.YELLOW
            )
        )
    else:
    ...
    if all_changes:
        ...
        with open(checklist_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                "This file lists all image paths referenced in the documentation.\n\n"
            )

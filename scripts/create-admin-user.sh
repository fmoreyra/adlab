...
create_admin_user() {
  ...
  if docker compose exec -T web python3 manage.py shell <<EOF; then
...
main() {
  ...
  if [ -z "$email" ]; then
    read -r -p "Enter admin email (default: admin@adlab.com): " email
    email="${email:-admin@adlab.com}"
  fi

  # Prompt for first name if not provided
  if [ -z "$first_name" ]; then
    read -r -p "Enter first name (default: Admin): " first_name
    first_name="${first_name:-Admin}"
  fi

  # Prompt for last name if not provided
  if [ -z "$last_name" ]; then
    read -r -p "Enter last name (default: User): " last_name
